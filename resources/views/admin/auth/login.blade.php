    ...
    <div class="relative">
        ...
        <div class="mb-0 w-screen lg:mx-auto lg:w-[500px] card shadow-lg border-none shadow-slate-100 relative">
            <div class="!px-10 !py-12 card-body">
                <a>
                    <img src="{{env('APP_URL').'/'.ServiceFacade::getData()->with('images')->first()->images->where('type', 'logo')->first()->url ?? ""}}" alt="" class="hidden h-6 mx-auto dark:block">
                    <img src="{{env('APP_URL').'/'.ServiceFacade::getData()->with('images')->first()->images->where('type', 'logo')->first()->url ?? ""}}" alt="" class="block h-6 mx-auto dark:hidden">
                </a>

                <div class="mt-8 text-center">
                    <h4 class="mb-1 text-custom-500 dark:text-custom-500">Xoş gəlmisiniz!</h4>
                    <p class="text-slate-500 dark:text-zink-200">Admin paneldə işə davam etmək üçün davam etmək üçün daxil olun.</p>
                </div>

                <form action="{{ route('admin.login.post') }}" method="POST" class="mt-10" id="signInForm">
                    @csrf
                    <div class="mb-3">
                        <label for="name" class="inline-block mb-2 text-base font-medium">İstifadəçi adı</label>
                        <input type="text" id="name" name="name" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">
                        <div id="name-error" class="hidden mt-1 text-sm text-red-500">Please enter a valid email address.</div>
                    </div>
                    <div class="mb-3">
                        <label for="password" class="inline-block mb-2 text-base font-medium">Şifrə</label>
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            ...
                            <label for="checkboxDefault1" class="inline-block text-base font-medium align-middle cursor-pointer">Məni xatırla</label>
                        </div>
                        <div id="remember-error" class="hidden mt-1 text-sm text-red-500">Please check the "Remember me" before submitting the form.</div>
                    </div>
                    @if ($errors->has('error'))
                    <div class="mb-4 mt-4 text-sm text-red-600">
                        {{ $errors->first('error') }}
                    </div>
                    @endif

                    <div class="mt-10">
                        <button type="submit" class="w-full text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">Daxil ol</button>
                    </div>
                </form>
            </div>
        </div>
