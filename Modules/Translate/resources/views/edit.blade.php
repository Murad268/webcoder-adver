    ...
    <div
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16">Tərcüməni redaktə et</h5>
                </div>
            </div>
                ...
                <div class="card col-span-2">
                    <div class="card-body">
                        <form enctype="multipart/form-data" method="post"
                              action="{{ route('translate.update', $model->id) }}">
                            @csrf
                            @method('PATCH')
                            ...
                            <div style="border: 1px solid black; padding: 30px" class="mt-5 mb-4 tab-content">
                                ...
                                @endphp
                                @foreach($languages as $language)
                                    <div class="tab-pane {{ $isFirst ? 'block' : 'hidden' }}"
                                         id="{{ $language->code }}">
                                        <div class="grid grid-cols-1 gap-x-5 sm:grid-cols-2">
                                            <div class="mb-3">
                                                <label for="value_{{ $language->code }}"
                                                       class="inline-block mb-2 text-base font-medium">Tərcümə
                                                    ({{ $language->name }})</label>
                                                <textarea name="value[{{ $language->code }}]"
                                                          ...
                                                          rows="3">{{ old('value.' . $language->code, $model->getTranslation('value', $language->code)) }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                    @php
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" id="group" name="group" value="{{ old('group', $model->group) }}"
                                       class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500">
                            </div>
                            <div class="mb-3">
                                <label for="keyword" class="inline-block mb-2 text-base font-medium">Açar Söz</label>
                                <input type="text" id="keyword" name="keyword"
                                       value="{{ old('keyword', $model->keyword) }}"
                                       class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500">
                            </div>
